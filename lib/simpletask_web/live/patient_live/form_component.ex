defmodule SimpletaskWeb.PatientLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      >
        ...
        <.input_core field={@form[:legal_person]} type="text" label="Legal person" />
        <:actions>
          <.button phx-disable-with="Salvando...">Salvar Paciente</.button>
        </:actions>
      </.simple_form>
    </div>
    """
  ...
  def update(%{patient: patient} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(PatientQuery.change_patient(patient))
     end)}
  end

  @impl true
  def handle_event("validate", %{"patient_schema" => patient_params}, socket) do
    changeset = PatientQuery.change_patient(socket.assigns.patient, patient_params)
    {:noreply, assign(socket, form: to_form(changeset, action: :validate))}
  end

  def handle_event("save", %{"patient_schema" => patient_params}, socket) do
    save_patient(socket, socket.assigns.action, patient_params)
  end

  defp save_patient(socket, :edit, patient_params) do
    case PatientQuery.update_patient(socket.assigns.patient, patient_params) do
      {:ok, patient} ->
        notify_parent({:saved, patient})

        {:noreply,
         socket
         |> put_flash(:info, "Dados do paciente atualizados com sucesso")
  ...
  defp save_patient(socket, :new, patient_params) do
    case PatientQuery.create_patient(patient_params) do
      {:ok, patient} ->
        ...
        {:noreply,
         socket
         |> put_flash(:info, "Dados do paciente criados com sucesso")
         |> push_patch(to: socket.assigns.patch)}
